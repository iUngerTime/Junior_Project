...
namespace Database_Helpers
{
    public class IngredientData : iIngredientData
    {
        ...
        {//Personally I feel like a name should be passed in directly
            return _database.ExecuteQuery_NoReturnType(String.Format("INSERT INTO NEW_INGREDIENTS(IngredientName) VALUES({0});", SqlServerDataAccess.Sanitize(newIng.Name)));
        }
        ...
        {
            string query = String.Format("SELECT * FROM NEW_INGREDIENTS WHERE IngredientName='{0}';", SqlServerDataAccess.Sanitize(name));
        ...
        {
            return _database.ExecuteQuery_NoReturnType(String.Format("DELETE FROM NEW_INGREDIENTS WHERE IngredientName='{0}';", SqlServerDataAccess.Sanitize(oldIng.Name)));
        }
        ...
        public int AddIngredientToPantry(int PantryID, int IngredientID, string Measurement, double Quantity = 1.0f)
        {
            ...
            string query = String.Format("INSERT INTO PANTRY_INGREDIENTS VALUES ({0}, {1}, '{2}', {3}, '{4}');", PantryID, IngredientID, SqlServerDataAccess.Sanitize(ingr.Name), Quantity, Measurement);
